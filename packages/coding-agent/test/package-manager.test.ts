...
describe("DefaultPackageManager", () => {
	...
	beforeEach(() => {
		...
		// PR #5: subprocess cwd resolves macOS /var symlinks to /private/var.
		tempDir = join(realpathSync(tmpdir()), `pm-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
Content`,
	...
	describe("embedded Bun", () => {
		...
		it.each([
			...
			{ spec: "fixture", installed: "1.0.0", response: "malformed", update: true },
			{ spec: "fixture", installed: "1.0.0", response: undefined, update: true },
